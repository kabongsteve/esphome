...
void Climate::publish_state() {
  ...
  if (ESPHOME_LOG_LEVEL >= ESPHOME_LOG_LEVEL_DEBUG) {
    nxt += std::snprintf(buf + nxt, sizeof(buf) - nxt, "Sending state: ");
  }

  nxt += std::snprintf(buf + nxt, sizeof(buf) - nxt, "%s: Mode(%s)", this->name_.c_str(), LOG_STR_ARG(climate_mode_to_string(this->mode)));

  if (traits.has_feature_flags(climate::CLIMATE_SUPPORTS_ACTION)) {
    nxt += std::snprintf(buf + nxt, sizeof(buf) - nxt, ", Action(%s)", LOG_STR_ARG(climate_action_to_string(this->action)));
  }
  if (traits.get_supports_fan_modes() && this->fan_mode.has_value()) {
    nxt += std::snprintf(buf + nxt, sizeof(buf) - nxt, ", Fan Mode(%s)", LOG_STR_ARG(climate_fan_mode_to_string(this->fan_mode.value())));
  }
  if (!traits.get_supported_custom_fan_modes().empty() && this->has_custom_fan_mode()) {
    nxt += std::snprintf(buf + nxt, sizeof(buf) - nxt, ", Custom Fan Mode(%s)", this->custom_fan_mode_);
  }
  if (traits.get_supports_presets() && this->preset.has_value()) {
    nxt += std::snprintf(buf + nxt, sizeof(buf) - nxt, ", Preset(%s)", LOG_STR_ARG(climate_preset_to_string(this->preset.value())));
  }
  if (!traits.get_supported_custom_presets().empty() && this->has_custom_preset()) {
    nxt += std::snprintf(buf + nxt, sizeof(buf) - nxt, ", Custom Preset(%s)", this->custom_preset_);
  }
  if (traits.get_supports_swing_modes()) {
  ...
  if (traits.has_feature_flags(climate::CLIMATE_SUPPORTS_TARGET_HUMIDITY)) {
    nxt += std::snprintf(buf + nxt, sizeof(buf) - nxt, ", Target Humidity(%.0f%%)", this->target_humidity);
  }
  if (ESPHOME_LOG_LEVEL >= ESPHOME_LOG_LEVEL_DEBUG)
    ESP_LOGD(TAG, buf);
  else
