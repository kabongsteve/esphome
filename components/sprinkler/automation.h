...
template<typename... Ts> class SetCurrentValveDurationAction : public Action<Ts...> {
 public:
  explicit SetCurrentValveDurationAction(Sprinkler *a_sprinkler) : sprinkler_(a_sprinkler) {}

  TEMPLATABLE_VALUE(uint32_t, valve_run_duration)

  void play(Ts... x) override {
    this->sprinkler_->set_current_valve_duration(this->valve_run_duration_.value(x...));
  }
