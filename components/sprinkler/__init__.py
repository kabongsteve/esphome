...
SetCurrentValveDurationAction = sprinkler_ns.class_("SetCurrentValveDurationAction", automation.Action)
...
SPRINKLER_ACTION_SET_CURRENT_RUN_DURATION_SCHEMA = cv.Schema(
    {
        cv.Required(CONF_ID): cv.use_id(Sprinkler),
        cv.Required(CONF_RUN_DURATION): cv.templatable(cv.positive_time_period_seconds),
    }
...
@automation.register_action(
    "sprinkler.set_current_valve_duration",
    SetCurrentValveDurationAction,
    SPRINKLER_ACTION_SET_CURRENT_RUN_DURATION_SCHEMA,
)
async def sprinkler_set_current_valve_duration_to_code(config, action_id, template_arg, args):
    paren = await cg.get_variable(config[CONF_ID])
    var = cg.new_Pvariable(action_id, template_arg, paren)
    template_ = await cg.templatable(config[CONF_RUN_DURATION], args, cg.uint32)
    cg.add(var.set_valve_run_duration(template_))
